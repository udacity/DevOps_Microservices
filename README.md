...
- [Install CircleCI locally](https://circleci.com/docs/2.0/local-cli/).  You will need to make a [personal token](https://app.circleci.com/settings/user/tokens) in CircleCI to use API locally. API tokens can be given to application (or people), and gives that application or person permission to act as you and have full read- and write-permissions to the software (in this case CircleCI). So it is a similar idea to SSH key and sshing into a computer.  So just a special admin password to access software (think AWS credentials).
...
- **virtualenv**: Creates isolated Python environments for Python libraries. It works by installing a bunch of files in a directory (eg: env/), and then modifying the PATH environment variable to prefix it with a custom bin directory (eg: env/bin/). An exact copy of the python or python3 binary is placed in this directory. When looking for package, python will examine the PATH variable (which tells python/macos system where to find commands) in order of the path directories from beginning to end.  So, because *virtualenv* appends paths to the beginning, python will look for commands based on virtualenv's paths.
- **pyenv**: pyenv is used to isolate Python versions. For example, you may want to test your code against Python 2.7, 3.6, 3.7 and 3.8, so you'll need a way to switch between them. Once activated, it prefixes the PATH environment variable with ~/.pyenv/shims, where there are special files matching the Python commands (python, pip). These are not copies of the Python-shipped commands; they are special scripts that decide on the fly which version of Python to run based on the PYENV_VERSION environment variable, or the .python-version file, or the ~/.pyenv/version file. pyenv also makes the process of downloading and installing multiple Python versions easier, using the command pyenv install.
